...
import { useGetRolesListQuery, useGetLendersListQuery } from "../../api/admin/adminActionsApi";
...
import { setCurrentSelectedOrg } from "../../featuers/authSlice";
...
const AccesorFirms = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useGetLendersListQuery({ currentPage, rowsPerPage, searchText, orderColumn, sortOrder });
    ...
    <div className="user-card">
      ...
      >
        <UserAvatar
          theme={row?.avatarBg}
          text={findUpper(
            row.organization_name != null && row.organization_name != undefined ? row.organization_name : ""
          )}
          image=""
        ></UserAvatar>{" "}
  ...
  const ManageUsers = ({ row }) => {
    ...
    const viewOrgUsers = () => {
      const updatedRow = { ...row, org_type: "accesor" };
      dispatch(setCurrentSelectedOrg(updatedRow));
      navigateTo("/admin/organization-users");
    };
    return (
      <div className="user-card">
        {" "}
              ...
              <DropdownMenu end>
                <ul className="link-list-opt no-bdr">
                  <React.Fragment>
                    ...
                    <li>
                      <DropdownItem tag="a" href="#" onClick={() => viewOrgUsers()}>
